...
# If the UPDATE_SPARK_CONF_DIR dir is non-empty, copy the contents to $SPARK_HOME/conf
...
if [ -z ${SPARK_MASTER_ADDRESS+_} ]; then
	...
	# run the spark master directly (instead of sbin/start-master.sh) to link master and container lifecycle
